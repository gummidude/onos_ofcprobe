...
public class HostToHostIntentCompiler
    ...
    private Intent createPathIntent(Path path, Host src, Host dst,
        ...
        Set<IpAddress> srcIPs = src.ipAddresses();
        Iterator<IpAddress> srcIpIter = srcIPs.iterator();
        IpAddress srcIp = null;
        while (srcIpIter.hasNext()) {
            srcIp = srcIpIter.next();
        }
        Set<IpAddress> dstIps = dst.ipAddresses();
        Iterator<IpAddress> dstIpIter = dstIps.iterator();
        IpAddress dstIp = null;
        while (dstIpIter.hasNext()) {
            dstIp = dstIpIter.next();
        }


        TrafficSelector selector;
        if (srcIp != null && dstIp != null) {
            selector = builder(intent.selector()).matchEthType(Ethernet.TYPE_IPV4).matchIPSrc(srcIp.getIp4Address().toIpPrefix()).matchIPDst(dstIp.getIp4Address().toIpPrefix()).build();
        } else {
            selector = builder(intent.selector())
